...
const editClick = (event: MouseEvent) => {
	...
	if (
		...
		const rendered =
			forceQuerySelector<HTMLSpanElement>(contentBox)(".rendered_text");
			...
			<div style="text-align:right; height:2em">
				{options.map(([handler, text]) => (
					<button
						...
						style={OTHER_STYLE}
						onClick={(e) => {
							handler(e as MouseEvent).catch(console.error);
						}}
...
const saveClick = async (event: MouseEvent) => {
	...
	if (
		...
		await updateProfile();
		await closeUpShop(button);
	}
};

const previewClick = async (event: MouseEvent) => {
	const button = event.target as HTMLButtonElement;
...
const cancelClick = async (event: MouseEvent) => {
	const button = event.target as HTMLButtonElement;
	if (button.style.cursor !== "default") await closeUpShop(button);
};

const closeUpShop = async (button: HTMLButtonElement) => {
	if (
		button.dataset.alias !== undefined &&
		button.dataset.field !== undefined
	) {
		...
		const value = await parseMarkup(fieldValue);
...
export const main = async () => {
	...
	if (key !== null) {
		const [, response, response2] = await Promise.all([
			...
			fetchInPage({ url: "https://rateyourmusic.com/account/account_edit" }),
		]);
