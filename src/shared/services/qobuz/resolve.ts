...
const getCoverArt = (document_: Document) => {
    const url = document_.querySelector<HTMLImageElement>('img.album-cover__image')?.src ?? undefined
...
const gerReleaseData = (document_: Document) => {
    const releaseScript = document_.querySelectorAll<HTMLScriptElement>('script[type="application/ld+json"]')[1]
    if (releaseScript) {
...
const getTracks = (document_: Document) => {
    return [...document_.querySelectorAll('#playerTracks div.track__items')].map((element) => {
        const position = element.querySelector('div.track__item--number')?.textContent?.trim()
        const title = element.getAttribute('title')?.trim()
        const duration = normalizeDuration(element.querySelector('span.track__item--duration')?.textContent?.trim())
...
export const resolve: ResolveFunction = async (url) => {
    ...
    const date = releaseData.datePublished ? stringToDate(releaseData.datePublished) : undefined
